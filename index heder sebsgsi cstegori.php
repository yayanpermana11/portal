...
<body class="<?= isset($_COOKIE['darkMode']) && $_COOKIE['darkMode'] === '1' ? 'dark-mode' : '' ?>">
    <header class="header">
        <div style="display: flex; align-items: center; gap: 20px; flex-wrap: wrap;">
            <!-- Logo MIS (sekaligus tombol filter MIS) -->
            <div class="logo filter-logo" onclick="setCategory('MIS Al-Jamila')" title="Tampilkan link MIS Al-Jamila">
                <?php
                $logoFile = __DIR__ . '/img/logo.png';
                if (file_exists($logoFile)) {
                    echo '<img src="/img/logo.png" alt="Logo MIS" style="height:40px; border-radius:8px;">';
                } else {
                    echo '<span class="logo-icon">📘</span>';
                }
                ?>
                <span class="logo-text">MIS Al-Jamila</span>
            </div>
    
            <!-- Logo SMP (sekaligus tombol filter SMP) -->
            <div class="logo filter-logo" onclick="setCategory('SMP Plus Al-Jamila')" title="Tampilkan link SMP Plus Al-Jamila">
                <?php
                $logoFile2 = __DIR__ . '/img/logo2.png';
                if (file_exists($logoFile2)) {
                    echo '<img src="/img/logo2.png" alt="Logo SMP" style="height:40px; border-radius:8px;">';
                } else {
                    echo '<span class="logo-icon">📙</span>';
                }
                ?>
                <span class="logo-text">SMP Plus Al-Jamila</span>
            </div>
    
            <!-- Tombol kembali ke Semua (opsional) -->
            <!--<div class="logo filter-logo" onclick="setCategory('semua')" title="Tampilkan semua link">-->
            <!--    <span class="logo-icon">📚</span>-->
            <!--    <span class="logo-text">Semua Link</span>-->
            <!--</div>-->
        </div>
    
        <!-- Search box tetap di sini -->
    ...
    <script>
        ...
        function renderCategories() {
            const cats = ['semua', ...new Set(allLinks.map(l => l.category))].sort();
            const container = document.getElementById('categoryTabs');
            container.innerHTML = cats.map(cat => {
                const count = cat === 'semua' ? allLinks.length : allLinks.filter(l => l.category === cat).length;
        ...
        function renderLinks() {
            ...
            let filtered = allLinks.filter(l => {
                const matchCat = currentCat === 'semua' || l.category === currentCat;
                const matchSearch = !search || l.name.toLowerCase().includes(search) || l.url.toLowerCase().includes(search) || l.category.toLowerCase().includes(search);
            ...
            grid.innerHTML = filtered.map((l, i) => `
                <div class="link-card" style="animation-delay:${i*0.03}s" onclick="window.open('${escapeHTML(l.url)}', '_blank')">
                    ...
                    <div class="link-info">
                        ...
                        <span class="link-category">${escapeHTML(l.category)}</span>
                    </div>
                </div>
            `).join('');
        }
        ...
        function setCategory(cat) {
            currentCat = cat;
            // Reset pencarian agar tidak mengganggu filter
